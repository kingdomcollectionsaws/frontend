...
export default function OrderCard({data}) {
  ...
  return (
    <>
{ data&& data?.map((i)=>(<div onClick={()=>navigate(`/account/order/${data._id}`)}  className='flex align-center justify-around border shadow-lg mt-8 hover:scale-105'>
...
<div className='flex flex-col '>
  <p className='font-bold mb-2'>uu</p>
  <p>256gb</p>
  <p>red</p>
  <p className='font-semibold mb-2'>$1234</p>

</div>
<p className='font-bold'><span><AlbumIcon className='w-[1rem] h-[1rem] text-green-500'/></span>{i?.orderStatus}</p>
</div>
))}
